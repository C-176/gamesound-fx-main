...
function TitleBar({ onSettingsClick, onValorantToggle, showValorant, valorantConnected, teamMode, onTeamToggle }: TitleBarProps) {
  ...
  return (
    <div className="[-webkit-app-region:drag] shrink-0 bg-bg-secondary border-b-2 border-border-default">
      <div className="titlebar-accent-line" />
      <div className="px-3 py-2 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          <RetroTV size={18} color={themeColor.cyan} />
          <div className="flex flex-col min-w-0 leading-tight">
            <span className="text-lg font-pixel text-accent tracking-wider truncate">{copy.app.title}</span>
        ...
        <div className="[-webkit-app-region:no-drag] flex items-center gap-1">
          {onValorantToggle && (
            <button
              ...
              className={`win-btn ${showValorant ? 'border-accent bg-accent-dim text-accent' : valorantConnected ? 'border-accent-green bg-accent-green/10' : ''}`}
          ...
          {onTeamToggle && (
            <button
              ...
              className={`win-btn ${teamMode ? 'border-accent bg-accent-dim text-accent' : ''}`}
          ...
          <button onClick={onSettingsClick} className="win-btn" title={copy.settings.title}>
          ...
          <button onClick={minimize} className="win-btn" title="最小化">
          ...
          <button onClick={close} className="win-btn win-btn-danger text-accent-red" title="关闭">
