  ...
  return (
    <div className="fixed bottom-3 left-3 z-[9999] flex flex-col gap-2 items-start pointer-events-none">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className="pointer-events-auto border-2 border-accent-pink bg-bg-secondary/95 p-3 min-w-[240px] max-w-[320px] animate-[slide-up_0.2s_ease-out]"
          style={{ boxShadow: '4px 0 12px rgba(255, 77, 199, 0.3)' }}
        >
          <div className="flex items-center gap-1.5 mb-1.5">
            <Satellite size={10} color="#ff4dc7" />
            <span className="font-pixel text-xs text-accent-pink">{toast.label}</span>
          </div>
            ...
            <div className="flex flex-col gap-1">
              {toast.soundIds.map((sid, i) => (
                <button key={sid}
                  ...
                  className="w-full flex items-center justify-between px-2 py-1 border-2 border-accent-pink bg-accent-pink/10 cursor-pointer hover:bg-accent-pink hover:text-black transition-none rounded-none"
                >
                  <span className="font-pixel text-sm text-text-primary">▶ {getSoundName(sid)}</span>
                  <span className="font-pixel text-xs text-accent-pink">{i + 1}</span>
                </button>
              ))}
            </div>
          ) : (
            <div className="px-2 py-1.5 border-2 border-border-default bg-bg-tertiary">
              <span className="font-pixel text-sm text-text-secondary">(no sound bound)</span>
            </div>
          )}
        </div>
      ))}
